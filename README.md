...
本仓库是 Switch++ 的部分源码开放版本（source-available），不是完整开源发行版，也不是 OSI 定义的开源项目。公开仓库包含前端界面、Tauri 外壳、配置预览、厂商预设和公开文档；付费版所需的核心 native 实现、授权校验、本地配置写入、本地兼容网关、环境安装/卸载和部分诊断能力保留在私有 core 中。

`src-tauri/src/lib.rs` 会引用 `.private/agent-switch-private-core/src-tauri-core/src/lib.rs`。该私有 core 不随公开仓库分发，因此仅 clone 本仓库不能构建出完整可用的官方应用。普通用户请从发布页下载安装包；公开源码主要用于审阅公开界面、了解产品行为、提交 issue 或围绕公开层做贡献。

如果本地没有私有 core，测试脚本会跳过标记为 internal-only 的私库依赖测试；完整构建、完整测试和官方发布流程仅在维护者环境中运行。
